...
/*   Updated: 2020/06/22 21:37:56 by caking           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			replace_one_label_by_value(t_command *command,
int count, t_label_list *list, int bytes, int args_bytes)
{
	int found;
...
void			replace_labels_with_values(t_program *prog)
{
	...
	while (list)
	{
		...
		count = 0;
		count_args_len = -byte_count;
		byte_count += 1 + g_op_tab[list->command.op_code - 1].arg_types_code;
		while (count < g_op_tab[list->command.op_code - 1].args_num)
		{
			...
			list->command.types[count] == DIRECT_LABEL)
				replace_one_label_by_value(&list->command, count, prog->labels,
				byte_count, count_args_len + byte_count);
